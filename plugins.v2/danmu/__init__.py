...
class Danmu(_PluginBase):
    ...
    def _get_filter_module(self):
        """延迟导入过滤模块"""
        try:
            from app.plugins.danmu.danmaku_filter import KeywordBlocklist, DanmakuFilter
            return KeywordBlocklist, DanmakuFilter
        except ImportError as e:
            logger.error(f"导入过滤模块失败: {e}")
            return None, None

    def filter_get_categories(self) -> Dict[str, Any]:
        """获取所有过滤分类"""
        try:
            KeywordBlocklist, _ = self._get_filter_module()
            if KeywordBlocklist is None:
                return schemas.Response(success=False, message="过滤模块不可用")
            categories = KeywordBlocklist.get_categories()
            return schemas.Response(success=True, data={"categories": categories})
        except Exception as e:
            logger.error(f"获取过滤分类失败: {e}")
    ...
    def filter_query_keywords(self, keyword: str = None) -> Dict[str, Any]:
        """查询关键词"""
        try:
            if not keyword:
    ...
    def filter_remove_category(self, data: Dict[str, Any]) -> schemas.Response:
        ...
        try:
            ...
            logger.error(f"移除分类失败: {e}")
            return schemas.Response(success=False, message=f"移除失败: {str(e)}")

    def filter_get_blocked_users(self) -> Dict[str, Any]:
        """获取屏蔽用户列表"""
        try:
            _, DanmakuFilter = self._get_filter_module()
            if DanmakuFilter is None:
                return schemas.Response(success=False, message="过滤模块不可用")
            # 注意：用户状态是实例级的，这里仅能获取关键词库信息
            return schemas.Response(
                success=True,
                message="用户屏蔽状态为运行时数据，每次刮削重新计算",
                data={"note": "用户信用评分在每次弹幕刮削时实时计算，不在请求间持久化"}
            )
        except Exception as e:
            logger.error(f"获取屏蔽用户失败: {e}")
            return schemas.Response(success=False, message=f"获取失败: {str(e)}")

    def filter_get_warned_users(self) -> Dict[str, Any]:
        """获取警告用户列表"""
        return self.filter_get_blocked_users()
    
    def filter_get_stats(self) -> Dict[str, Any]:
        """获取过滤系统统计数据"""
        try:
            KeywordBlocklist, DanmakuFilter = self._get_filter_module()
            if KeywordBlocklist is None:
                return schemas.Response(success=False, message="过滤模块不可用")
            categories = KeywordBlocklist.get_categories()
            blocked_users = DanmakuFilter.get_blocked_users() if DanmakuFilter else []
            return schemas.Response(
                success=True,
                data={
                    "category_count": len(categories),
                    "keyword_count": sum(len(c.get("keywords", [])) for c in categories),
                    "enabled_category_count": sum(1 for c in categories if c.get("enabled")),
    ...
    def get_scrape_history(self) -> Dict[str, Any]:
        """获取刮削历史记录"""
        try:
            history = []
            # 从刮削进度中获取最近的文件
            ...
            # 从重试任务中提取历史信息
            with self._retry_lock:
                for file_path, task_info in list(self._retry_tasks.items())[-20:]:
                    history.append({
                        "file_path": file_path,
                        "file_name": os.path.basename(file_path) if file_path else "未知",
                        "danmu_count": task_info.get("last_danmu_count", 0),
                        "time": task_info.get("last_attempt", "").isoformat() if task_info.get("last_attempt") else "",
                        "status": "failed" if task_info.get("retry_count", 0) > 0 else "success",
                    })
            ...
            if progress.get("current_file"):
                history.insert(0, {
                    ...
                    "file_name": os.path.basename(progress["current_file"]),
                    "danmu_count": 0,
                    "time": datetime.now().isoformat(),
                    "status": "running",
                })
            
            return schemas.Response(success=True, data=history[-50:])
        except Exception as e:
            logger.error(f"获取刮削历史失败: {e}")
            return schemas.Response(success=False, message=f"获取失败: {str(e)}", data=[])
