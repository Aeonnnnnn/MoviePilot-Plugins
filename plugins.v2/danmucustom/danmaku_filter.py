...
# 模块级存储：保存最后一次的 DanmakuFilter 实例，供 API 查询
_last_filter_instance: Optional['DanmakuFilter'] = None
# 持久化恢复：插件启动时注入的用户状态，DanmakuFilter 初始化时自动加载
_pending_restored_users: Optional[Dict[str, dict]] = None


def set_pending_restored_users(users: Optional[Dict[str, dict]]):
    """注入持久化恢复的用户状态，供下次 DanmakuFilter 初始化时自动加载。"""
    global _pending_restored_users
    _pending_restored_users = users


def pop_restored_users() -> Optional[Dict[str, dict]]:
    """获取持久化恢复的用户状态（非一次性消费）。

    返回 _pending_restored_users 的浅拷贝，不清除模块级变量，
    确保批量任务中每个 DanmakuFilter 实例都能基于最新持久化状态初始化。
    """
    global _pending_restored_users
    if _pending_restored_users is None:
        return None
    # 返回浅拷贝，避免调用方意外修改模块级状态
    return dict(_pending_restored_users)


# ============ 跨实例累计统计：避免模块级单实例覆盖，支持批量刮削累计与持久化 ============
_cumulative_stats: Dict[str, int] = {}
_cumulative_scrapes: int = 0
_pending_cumulative_stats: Optional[Dict[str, Any]] = None

# 需要累加的数值字段（与 DanmakuFilter.stats 对应）
_CUM_FIELDS = [
    "total_input", "mode_blocked", "keyword_blocked", "user_blocked",
    "frequency_blocked", "length_blocked", "symbol_blocked",
    "repeat_char_blocked", "burst_duplicate_blocked", "similarity_blocked",
    "density_removed", "total_output",
]

def set_pending_cumulative_stats(data: Optional[Dict[str, Any]]):
    """设置待恢复的累计统计（插件启动/重载时由持久化数据注入）。"""
    global _pending_cumulative_stats
    _pending_cumulative_stats = data

def pop_pending_cumulative_stats() -> Optional[Dict[str, Any]]:
    """获取待恢复的累计统计（非一次性消费），避免重复注入。"""
    global _pending_cumulative_stats
    if _pending_cumulative_stats is None:
        return None
    return dict(_pending_cumulative_stats)

def reset_cumulative_stats():
    """重置累计统计（仅当用户明确清空时调用，如 filter_reset_stats API）。"""
    global _cumulative_stats, _cumulative_scrapes, _pending_cumulative_stats
    _cumulative_stats = {}
    _cumulative_scrapes = 0
    _pending_cumulative_stats = None

def _restore_cumulative_if_needed():
    """若累计为空且有待恢复数据，则从 pending 恢复到模块级累计（非覆盖式）。"""
    global _cumulative_stats, _cumulative_scrapes
    if _cumulative_scrapes == 0 and _pending_cumulative_stats:
        data = _pending_cumulative_stats
        for f in _CUM_FIELDS:
            _cumulative_stats[f] = int(data.get("stats", {}).get(f, 0))
        _cumulative_scrapes = int(data.get("scrapes", 0))

def accumulate_stats(instance_stats: Dict[str, int]):
    """将单次过滤统计累加到模块级累计（跨实例/跨文件持续累计，非单实例覆盖）。"""
    global _cumulative_stats, _cumulative_scrapes
    _restore_cumulative_if_needed()
    for f in _CUM_FIELDS:
        _cumulative_stats[f] = _cumulative_stats.get(f, 0) + int(instance_stats.get(f, 0))
    _cumulative_scrapes += 1

def get_cumulative_stats() -> Dict[str, Any]:
    """返回跨实例累计统计，含刮削次数与各阶段移除数。"""
    _restore_cumulative_if_needed()
    return {
        "scrapes": _cumulative_scrapes,
        "stats": dict(_cumulative_stats),
        "restored": _cumulative_scrapes > 0 or any(v > 0 for v in _cumulative_stats.values()),
    }


def get_last_filter() -> Optional['DanmakuFilter']:
    """获取最后一次 DanmakuFilter 实例"""
    return _last_filter_instance
...
class KeywordBlocklist:
    ...
    def get_categories(cls) -> Dict:
        ...
        return {name: {
            ...
            "combos": [list(r) for r in info.get("combo_rules", [])],
            "regexes": [list(r) for r in info.get("regex", [])],
    ...
    @classmethod
    def remove_regex_rule(cls, category: str, pattern: str) -> bool:
        """从指定分类移除正则规则（按 pattern 精确匹配）"""
        if category in cls.CATEGORIES:
            info = cls.CATEGORIES[category]
            before = len(info.get("regex", []))
            info["regex"] = [r for r in info.get("regex", []) if r[0] != pattern]
            return len(info.get("regex", [])) != before
        if category in cls._user_categories:
            info = cls._user_categories[category]
            before = len(info.get("regex", []))
            info["regex"] = [r for r in info.get("regex", []) if r[0] != pattern]
    ...
    @classmethod
    def remove_combo_rule(cls, category: str, keyword: str) -> bool:
        """从指定分类移除组合规则（按关键词精确匹配）"""
        if category in cls.CATEGORIES:
            info = cls.CATEGORIES[category]
            before = len(info.get("combo_rules", []))
            info["combo_rules"] = [r for r in info.get("combo_rules", []) if r[0] != keyword]
            return len(info.get("combo_rules", [])) != before
        if category in cls._user_categories:
            info = cls._user_categories[category]
            before = len(info.get("combo_rules", []))
            info["combo_rules"] = [r for r in info.get("combo_rules", []) if r[0] != keyword]
...
class UserRecord:
    ...
    def to_dict(self) -> dict:
        """将 UserRecord 序列化为可 JSON 存储的字典，用于持久化恢复。"""
        return {
            "mid_hash": self.mid_hash,
            "credit_score": self.credit_score,
            "total_danmaku": self.total_danmaku,
            "violation_count": self.violation_count,
            "violation_reasons": list(self.violation_reasons),
            "sample_contents": list(self.sample_contents),
            "first_seen": self.first_seen,
            "last_seen": self.last_seen,
            "is_blocked": self.is_blocked,
            "blocked_at": self.blocked_at,
            "consecutive_good": self.consecutive_good,
            "good_since_blocked": self.good_since_blocked,
            "last_violation_at": self.last_violation_at,
        }

    @staticmethod
    def from_dict(d: dict) -> "UserRecord":
        """从字典恢复 UserRecord 对象，用于持久化恢复。"""
        return UserRecord(
            mid_hash=d.get("mid_hash", ""),
            credit_score=d.get("credit_score", 100),
            total_danmaku=d.get("total_danmaku", 0),
            violation_count=d.get("violation_count", 0),
            violation_reasons=d.get("violation_reasons", []),
            sample_contents=d.get("sample_contents", []),
            first_seen=d.get("first_seen", 0.0),
            last_seen=d.get("last_seen", 0.0),
            is_blocked=d.get("is_blocked", False),
            blocked_at=d.get("blocked_at", 0.0),
            consecutive_good=d.get("consecutive_good", 0),
            good_since_blocked=d.get("good_since_blocked", 0),
            last_violation_at=d.get("last_violation_at", 0.0),
        )
...
class UserTracker:
    ...
    def __init__(
        ...
        """初始化用户信用追踪器：维护用户弹幕记录、信用分、频率检测参数与样本存储上限。"""
    ...
    def export_state(self) -> Dict[str, dict]:
        """将全部用户信用/封禁/违规数据导出为可 JSON 序列化的字典，供插件持久化。"""
        return {mid_hash: user.to_dict() for mid_hash, user in self._users.items()}

    def restore_state(self, state: Dict[str, dict]):
        """从持久化状态恢复用户信用/封禁/违规数据。"""
        if not state:
            return
        for mid_hash, d in state.items():
            self._users[mid_hash] = UserRecord.from_dict(d)
...
# ---------- 相似弹幕过滤的辅助函数 ----------
# 这些函数被设计为纯函数，避免依赖 MoviePilot 任何全局服务。
# 相似过滤的核心性能问题：若对所有弹幕做两两 _text_similarity 比较，
# 复杂度退化为 O(n^2)。优化思路（不降低原功能的去重效果）：
#   - 先做文本规范化 + dict 计数，把完全重复弹幕合并（O(n)）；
#   - 再做“双字 shingle 倒排索引”做全局候选生成（匹配范围与原算法一致，
#     仍是对全部弹幕做相似比较，不做时间分桶，避免漏掉不同时段的相似弹幕）；
#   - 仅对少量候选弹幕调用原始 _text_similarity 做最终判定，
#     阈值语义与原实现完全一致，因此去重效果保持不变。

def _sim_normalize(text: str) -> str:
    """相似过滤专用规范化：小写、去全角空格/标点、仅保留中文/字母/数字、压缩空白。"""
    if not text:
        return ""
    text = str(text).lower()
    # 全角转半角空格/标点
...
def _sim_bigrams(text: str) -> set:
    """从规范化文本中提取 2-gram 集合，用于倒排索引候选生成。"""
    s = _sim_normalize(text)
    if len(s) <= 1:
        return set()
...
class AdvancedFilters:
    ...
    def filter_similar_content(
        ...
        性能优化（避免 O(n^2) 全量两两比较，同时**保持原功能的去重效果**）：
        1. 文本规范化 + dict 计数，先把完全重复弹幕合并（O(n)）；
        2. 用“双字 shingle 倒排索引”做**全局**候选生成（不按时间分桶，
           匹配范围与原算法一致，不同时段的相似弹幕同样会被合并）；
        3. 仅对少量候选弹幕调用原始 _text_similarity 做最终判定，
           阈值语义与原实现完全一致，因此去重效果不变。
        ...
        Args:
            ...
            time_window: 保留参数兼容（当前为保留效果不做时间分桶，默认 0）
        ...
        n = len(danmakus)
        # 预计算长度，用于长度带预筛选（与 _text_similarity 内部 early-return 语义一致，
        # 跳过长度差异过大必然不相似的候选，避免昂贵的精确相似度计算）
        lens = [len(danmakus[i].get("content", "")) for i in range(n)]
        ...
        # ---- 第 1 步：规范化 + 完全重复去重（dict 计数，O(n)）----
        norm_map: Dict[str, int] = {}
        for i, dm in enumerate(danmakus):
            key = _sim_normalize(dm.get("content", ""))
            if not key:
                continue
            if key in norm_map:
                union(i, norm_map[key])
            else:
                norm_map[key] = i

        # ---- 第 2 步：双字 shingle 倒排索引做全局候选生成 ----
        # 倒排索引：bigram -> [出现该 bigram 的弹幕索引列表]
        inverted: Dict[str, List[int]] = {}
        bigrams_list: List[set] = []
        for i, dm in enumerate(danmakus):
            bg = _sim_bigrams(dm.get("content", ""))
            bigrams_list.append(bg)
            for g in bg:
                inverted.setdefault(g, []).append(i)

        # 超高频 bigram（几乎出现在所有弹幕中）对去重无意义，且会引爆候选集，
        # 直接跳过其倒排桶（identical 文本已在上一步 dict 去重处理）。
        max_bucket = max(1, n // 3)
        # 单个倒排桶过大时，只抽样若干候选（均匀跨越桶），在控制复杂度的同时
        # 仍能覆盖到近重复（完全相同的文本已由 dict 去重处理）。
        per_bucket_cap = 64

        # ---- 第 3 步：候选生成 + 精确 _text_similarity 验证（与原阈值语义一致）----
        for i in range(n):
            bg_i = bigrams_list[i]
            if not bg_i:
                continue
            candidates = set()
            for g in bg_i:
                bucket = inverted.get(g)
                if not bucket or len(bucket) > max_bucket:
                    continue
                step = 1 if len(bucket) <= per_bucket_cap else len(bucket) // per_bucket_cap
                for j in bucket[::step]:
                    if j > i:
                        candidates.add(j)
            for j in candidates:
                content_a = danmakus[i].get("content", "")
                content_b = danmakus[j].get("content", "")
                if content_a == content_b:
                    union(i, j)
                    continue
                # 长度带预筛选：与 _text_similarity 内部 early-return 语义一致，跳过必不相似者
                la, lb = lens[i], lens[j]
                if la and lb:
                    _min = la if la < lb else lb
                    if abs(la - lb) > _min * 0.8:
                        continue
                # 廉价预筛选：至少需要 2 个共同 bigram，否则几乎不可能相似，
                # 直接跳过昂贵的精确相似度计算（真实近重复短串至少共享 2 个 bigram）
                if len(bg_i & bigrams_list[j]) < 2:
                    continue
                if AdvancedFilters._text_similarity(content_a, content_b) >= similarity_threshold:
                    union(i, j)
...
class DanmakuFilter:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        # 持久化恢复：注入插件上次保存的用户信用/封禁状态
        restored_users = pop_restored_users()
        if restored_users:
            self.user_tracker.restore_state(restored_users)
            logger.info(f"已从持久化恢复 {len(restored_users)} 个用户过滤状态")
        ...
        # 保存到模块级变量，供 API 查询
        global _last_filter_instance
        _last_filter_instance = self

        # 持久化恢复后，将当前用户状态写回 _pending_restored_users，
        # 确保后续创建的 DanmakuFilter 实例能基于最新状态初始化
        global _pending_restored_users
        if _pending_restored_users is not None:
            _pending_restored_users = self.user_tracker.export_state()

        # 累计统计：恢复插件持久化的历史累计（仅本次进程尚未累计时）
        _restore_cumulative_if_needed()
    ...
    def filter(self, danmakus: List[Dict], verbose: bool = False) -> List[Dict]:
        ...
        for dm in danmakus:
            ...
            if is_blocked:
                ...
                continue
            stage0.append(dm)
        timings["mode"] = (time.perf_counter() - _t) * 1000
        _t = time.perf_counter()
        ...
        for dm in stage0:
            ...
            stage1.append(dm)
        timings["length_symbol_repeat"] = (time.perf_counter() - _t) * 1000
        _t = time.perf_counter()
        ...
        timings["user_state"] = (time.perf_counter() - _t) * 1000
        _t = time.perf_counter()
        ...
        if burst_blocked:
            ...
            stage2 = stage1_freq
        timings["burst_duplicate"] = (time.perf_counter() - _t) * 1000
        _t = time.perf_counter()
        ...
        for dm in stage2:
            ...
            stage3.append(dm)
        timings["keyword"] = (time.perf_counter() - _t) * 1000
        _t = time.perf_counter()
        ...
        timings["similarity"] = (time.perf_counter() - _t) * 1000
        _t = time.perf_counter()
        ...
        timings["density"] = (time.perf_counter() - _t) * 1000

        if verbose:
            print(f"  密度删减: {len(stage4)} → {len(stage5)} (移除{len(stage4)-len(stage5)}条)")

        self.stats["total_output"] = len(stage5)
        self.stats["stage_timings"] = timings

        total_ms = sum(timings.values())
        self.logger.info(
            f"弹幕过滤完成: 输入{len(danmakus)} → 输出{len(stage5)} | "
            f"各阶段耗时(ms): mode={timings['mode']:.1f} "
            f"length/symbol/repeat={timings['length_symbol_repeat']:.1f} "
            f"user_state={timings['user_state']:.1f} "
            f"burst={timings['burst_duplicate']:.1f} "
            f"keyword={timings['keyword']:.1f} "
            f"similarity={timings['similarity']:.1f} "
            f"density={timings['density']:.1f} | 总计{total_ms:.1f}ms"
        )

        # 累加到跨实例统计（供 API 查询全局累计，而非仅最后实例）
        accumulate_stats(self.stats)
    ...
    def unblock_user(self, mid_hash: str) -> bool:
        """解除用户封禁状态，重置信用分到初始值"""
        if not mid_hash or mid_hash not in self.user_tracker._users:
            # 尝试模糊匹配（只传了前12位的情况）
            for mh in list(self.user_tracker._users.keys()):
                if mh.startswith(mid_hash):
                    mid_hash = mh
                    break
            else:
                return False
        user = self.user_tracker._users[mid_hash]
        user.is_blocked = False
        user.blocked_at = 0.0
        user.credit_score = self.user_tracker.config.INITIAL_SCORE
        user.consecutive_good = 0
        user.good_since_blocked = 0
        return True

    def reset_user_credit(self, mid_hash: str, score: int = None) -> bool:
        """重置用户信用分（默认恢复到初始值）"""
        if not mid_hash or mid_hash not in self.user_tracker._users:
            for mh in list(self.user_tracker._users.keys()):
                if mh.startswith(mid_hash):
                ...
                return False
        user = self.user_tracker._users[mid_hash]
        user.credit_score = score if score is not None else self.user_tracker.config.INITIAL_SCORE
        user.is_blocked = False
        user.blocked_at = 0.0
        user.consecutive_good = 0
        user.good_since_blocked = 0
    ...
    def query_all_users(self) -> List[Dict]:
        ...
        return [
            self.user_tracker.get_user_stats(mh)
            for mh in self.user_tracker._users.keys()
        ]

    def export_filter_state(self) -> Dict[str, dict]:
        """导出用户信用/封禁/违规数据，供插件持久化保存。"""
        return self.user_tracker.export_state()
